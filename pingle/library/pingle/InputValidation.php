...
class InputValidation {
    ...
    public function run() {
        ...
        foreach (new \ArrayIterator($this->validation_rules) as $var => $opt) {
            ...
            //fix the warning
            if ($opt['required'] == true && !isset($this->source[$var])) {
                continue;
            }
     ...
     *
     */
    public function addRule($varname, $type, $required = false, $min = false, $max = false, $trim = false, $regex = "") {
     ...
     *
     */
    private function validateNumeric($var, $min = false, $max = false, $required = false) {
        if ($required == false && strlen($this->source[$var]) == 0) {
            return true;
        }
