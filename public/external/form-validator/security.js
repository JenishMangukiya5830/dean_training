(function ($, window) {
    "use strict";
    $.formUtils.addValidator({name: "spamcheck", validatorFunction: function (val, $el, config) {
            var attr = $el.valAttr("captcha");
            return attr === val
        }, errorMessage: "", errorMessageKey: "badSecurityAnswer"});
    $.formUtils.addValidator({name: "confirmation", validatorFunction: function (value, $el, config, language, $form) {
    ...
    $.formUtils.addValidator({name: "creditcard", validatorFunction: function (value, $el, config, language, $form) {
    ...
    $.formUtils.addValidator({name: "cvv", validatorFunction: function (val) {
    ...
    $.formUtils.addValidator({name: "strength", validatorFunction: function (val, $el, conf) {
            ...
            $el.bind("keyup", function () {
                ...
                var css = {background: "pink", color: "#FF0000", fontWeight: "bold", border: "red solid 1px", borderWidth: "0px 0px 4px", display: "inline-block", fontSize: config.fontSize, padding: config.padding};
                var text = config.bad;
                if (strength == 1) {
                    text = config.weak
                } else if (strength == 2) {
    ...
    var requestServer = function (serverURL, $element, val, conf, callback) {
        ...
        $.ajax({url: serverURL, type: "POST", cache: false, data: reqParams, dataType: "json", error: function (error, err) {
    ...
    $.formUtils.addValidator({name: "server", validatorFunction: function (val, $el, conf, lang, $form) {
